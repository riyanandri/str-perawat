...
class JenisPkController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        $nama_pk = $request->input('nama_pk'); //tangkap parameter id yang di post

        $cek = JenisPk::where('nama_pk', '=', $nama_pk)->count(); //query cek id apakah sudah ada di tabel atau belum
        if($cek) {
            return response()->json([ //respon json jika gagal
                'status' => false,
                'info' => "Nama PK tersebut sudah ada di database"
            ], 201);
            return false;
        }

        $post = JenisPk::create($inputclear); //jika lolos pengecekan id maka query insert ini jalan 
    ...
    public function edit(Request $request)
    {
        ...
        $post  = JenisPk::where($where)->first(); //get ke tabel di model berdasarkan id
